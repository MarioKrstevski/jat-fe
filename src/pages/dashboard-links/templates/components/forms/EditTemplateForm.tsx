...
export default function EditTemplateForm({}: EditTemplateFormProps) {
  const dialogControl = useDialogControl();
  const queryClient = useQueryClient();

  const activeTemplate =
    dialogControl.modals.editTemplate?.data?.template;

  const { mutateAsync: editTemplate } = useMutation({
    //  @ts-ignore
    mutationFn: api.templates.editTemplate,
    onSuccess: (updatedTemplate) => {
      //   queryClient.invalidateQueries({ queryKey: ["templates"] });
      queryClient.setQueryData(["templates"], (oldData: any) => {
        return oldData.map((d: Template) =>
          d.id === updatedTemplate.id ? updatedTemplate : d
        );
      });
      dialogControl.closeModal("editTemplate");
